...
$resultat = file("todo.json");
$array_json = json_decode($resultat, true);
// print_r($resultat);
print_r($_POST);
...
// if ($new_array = ["faire" => $newstr]) {
// 			$array_etat_faire = ["etat" => "faire"] 
// 			array_push ($new_array, $array_etat_faire);
// }else {
// 	$array_etat_archives = ["etat" => "archives"] 
// 			array_push ($new_array, $array_etat_archives);
// };
 ...
 <body>
 	...
 	<fieldset>
 		...
 		foreach ($array_json as $key => $i) {
 			 ...
 			 echo '<input class="barré" type="checkbox" name="' . $key . '" value="' . $i["archives"] . '"> <label>' . $i["archives"] . '</label>' . '</br>';
 		};
